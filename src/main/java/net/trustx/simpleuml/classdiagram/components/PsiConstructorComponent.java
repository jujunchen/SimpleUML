   ...
   extends JPanel
 {
   ...
   public PsiConstructorComponent(ClassDiagramComponent classDiagramComponent, PsiClassComponent psiClassComponent, PsiMethod psiMethodConstructor) {
     setOpaque(false);
     setLayout(new BoxLayout(this, 0));

     PsiModifierList psiModifierList = psiMethodConstructor.getModifierList();
     this.accessLabel = new UMLLabel(UMLUtils.getUMLModifierString(psiModifierList, classDiagramComponent.getDiagramSettings().isLongModifier()));
     ...
     EditorHelper.initQuickSourceSupport(classDiagramComponent.getDiagramSettings().getQuickSourceLinkColor(), classDiagramComponent.getProject(), (JComponent)psiClassComponent, this.constructorNamelabel, psiMethodConstructor.getNameIdentifier(), underscored, false, false);
     ...
     for (int i = 0; i < psiParameters.length; i++) {
       ...
       EditorHelper.initQuickSourceSupport(classDiagramComponent.getDiagramSettings().getQuickSourceLinkColor(), classDiagramComponent.getProject(), (JComponent)psiClassComponent, paramNameLabel, psiParameter.getNameIdentifier(), underscored, false, false);
       ...
       PsiTypeComponent typeComponent = new PsiTypeComponent(classDiagramComponent, (JComponent)psiClassComponent, psiParameter.getType(), font, underscored, true, showParameters);
